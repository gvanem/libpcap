...
#define __drv_allocatesMem(object)
#endif

#ifndef __drv_freesMem
#define __drv_freesMem(object)
#endif
...
typedef struct {
   ...
   */
  static const GUID local_GUID_DEVINTERFACE_USB_HOST_CONTROLLER = {
                    0x3abf6f2d, 0x71c4, 0x462a,
                    { 0x8a, 0x92, 0x1e, 0x68, 0x61, 0xe6, 0xaf, 0x27 }
                  };
  #define GUID_DEVINTERFACE_USB_HOST_CONTROLLER local_GUID_DEVINTERFACE_USB_HOST_CONTROLLER
#endif
...
{
    a->array = malloc (initial_size * sizeof(LPTSTR));
    a->used = 0;
    a->size = initial_size;
}
...
static void insert_string_array (StringArray *a, LPTSTR hwid)
{
    ...
    {
      a->size *= 2;
      a->array = realloc(a->array, a->size * sizeof(LPTSTR));
    }
...
static BOOL is_standard_hwid (LPTSTR hwid)
{
    ...
    if (_tcscmp("USB\\ROOT_HUB", hwid) == 0)
       return TRUE;

    if (_tcscmp("USB\\ROOT_HUB20", hwid) == 0)
       return TRUE;
...
static LPTSTR * GetMultiSzIndexArray(__drv_aliasesMem LPTSTR MultiSz)
{
    LPTSTR scan;
    LPTSTR *array;
    int elements;
 ...
 */
static void DelMultiSz(__drv_freesMem(object) LPTSTR* Array)
{
    if(Array)
    {
        Array--;
...
static void find_non_standard_hwids (HDEVINFO devs,
...
static void restart_device (HDEVINFO devs,
                            PSP_DEVINFO_DATA devInfo,
                            PSP_DEVINFO_LIST_DETAIL_DATA devInfoListDetail)
{
...
void init_non_standard_roothub_hwid (void)
{
    int length;
    PTSTR multi_sz;
...
void restart_all_usb_devices (void)
{
